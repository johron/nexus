...
struct event_broker {
	struct cancel_token {
		...
		[[nodiscard]] bool is_detached() const { return m_system == nullptr; }
	...
	struct listener_token {
		...
		listener_token(const listener_token& other) { *this = other; }

		listener_token(listener_token&& other) { *this = std::forward<listener_token>(other); }
		...
		[[nodiscard]] bool is_valid() const { return m_token && !m_token->is_detached(); }
