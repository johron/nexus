...
enum class facing : int32_t { right, down, left, up };
...
struct sprite_sheet {
	...
	auto make_animation(facing facing, nexus::vector2i origin, uint8_t frames) {
		...
		return std::make_unique<util::animation>(m_image, rects, std::chrono::milliseconds(300));
	}

	auto make_animation(entity entity, facing facing) {
		switch (entity) {
